...
def rnn_step_backward(dnext_h, cache):
    ...
    dz = dnext_h * (1 - torch.pow(torch.tanh(z),2))
...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        dhh = dprev_h + dh[:, t, :]
        dx[:,t,:], dprev_h, dWxx, dWhh, dbb = rnn_step_backward(dhh, cache[t])
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    loss = F.cross_entropy(
        x.reshape(-1, V), y.reshape(-1), ignore_index=ignore_index, reduction='none'
    )
    loss = loss.view(N, T)
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = WordEmbedding(vocab_size=vocab_size, embed_size=wordvec_dim)
    ...
    def forward(self, images, captions):
        ...
        # step 1.
        image_features = self.image_encoder(images)     # (N, C, hh, ww) (B, out_channels, H / 32, W / 32)
        N, C, _, _ = image_features.shape
        if self.cell_type in ('rnn','lstm'):
            image_features = image_features.view(N, C, -1).mean(dim=-1) # (N, C)
    ...
    def sample(self, images, max_length=15):
        ...
        image_features = self.image_encoder(images) # (N, C, H, W)
        N, C, _, _ = image_features.shape
        if self.cell_type in ('rnn','lstm'):
            image_features = image_features.view(N, C, -1).mean(dim=-1) # (N, C)
            h0 = self.feature_proj(image_features)
            prev_h = h0
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        A = x.mm(self.Wx) + prev_h.mm(self.Wh) + self.b.view(1, -1)   # (N, 4H)

        H = prev_h.shape[1]
        i = F.sigmoid(A[:,0:H])
        f = F.sigmoid(A[:,H:2*H])
        o = F.sigmoid(A[:,2*H:3*H])
        cc = F.tanh(A[:,3*H:])

        # compute
        next_c = f * prev_c + i * cc
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        hn = torch.zeros(size=(N,T,H), device=h0.device)
        for t in range(T):
            attn, _ = dot_product_attention(prev_h=prev_h, A=A)
