...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        attn_weight = torch.zeros(size=(K,), device=query.device)
        for j in range(K):
            attn_weight[j] = query[i].dot(key[j])
        attn_weight = F.softmax(attn_weight / (M ** 0.5), dim=0)
        out[i] = attn_weight.view(1, K).mm(value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    out = torch.zeros_like(query)
    for i in range(K):
        attn_weight = torch.zeros(size=(N, K), device=query.device)
        for j in range(K):
            attn_weight[:,j] = query[:,i,:].view(N,1,M).bmm(key[:,j,:].view(N,M,1)).view(N)
        attn_weight = F.softmax(attn_weight / (M ** 0.5), dim=1)    # (N,K)
        out[:,i,:] = (attn_weight.view(N,1,K).bmm(value)).view(N,M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    attn_weight = query.bmm(key.transpose(1,2)) / (M ** 0.5)
    if mask is not None:
        ##########################################################################
    ...
    # Replace "pass" statement with your code
    weights_softmax = F.softmax(attn_weight, dim=-1)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # init weight
        c1 = (6 / (dim_in + dim_q)) ** 0.5
        c2 = (6 / (dim_in + dim_v)) ** 0.5
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.heads = nn.ModuleList(
            [SelfAttention(dim_in=dim_in,dim_q=dim_out,dim_v=dim_out) for _ in range(self.num_heads)]
        )
        
        self.output_transform = nn.Linear(self.num_heads * dim_out, dim_in)
    ...
    def forward(
        ...
        for head in self.heads:
            y.append(head(query,key,value,mask))    # each of y is shape of (N, K, M)

        y = torch.cat(y, dim=-1)  # (N, K, h*M) cat 是续接，而 stack是叠加
        y = self.output_transform.forward(y.view(N*K,-1))   #(N*K, h*M) -> (N*K,M)
        y = y.view(N, K, M)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.first_f = nn.Linear(inp_dim, hidden_dim_feedforward)
        self.second_f = nn.Linear(hidden_dim_feedforward, inp_dim)
        c = (6 / (inp_dim + hidden_dim_feedforward)) ** 0.5
        nn.init.uniform_(self.first_f.weight, a=-c, b=c)
        nn.init.uniform_(self.second_f.weight, a=-c, b=c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(self, x):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        out_f1 = self.first_f(x)
        y = self.second_f(torch.max(torch.zeros_like(out_f1), out_f1))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.MultiHeadAttention = MultiHeadAttention(num_heads=num_heads, dim_in=emb_dim, dim_out=emb_dim//num_heads)
        self.layer_norm_1 = LayerNormalization(emb_dim=emb_dim)
        self.layer_norm_2 = LayerNormalization(emb_dim=emb_dim)
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out1 = self.MultiHeadAttention.forward(x, x, x)
        out2 = self.dropout(self.layer_norm_1(out1 + x))
        out3 = self.ffn(out2)
        y = self.dropout(self.layer_norm_2(out2 + out3))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    pos = torch.arange(K, dtype=torch.float).reshape(1, -1, 1)  # (1,K,1)
    dim = torch.arange(M, dtype=torch.float).reshape(1, 1, -1)  # (1,1,M)
    phase = pos / (1e4 ** (torch.div(dim, M,rounding_mode='floor')))    # (1,K,M)
