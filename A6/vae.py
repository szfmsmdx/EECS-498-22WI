...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        # Replace "pass" statement with your code
        tmp = self.encoder(x)
        mu = self.mu_layer(tmp)
        logvar = self.logvar_layer(tmp)
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # Replace "pass" statement with your code
        N = x.shape[0]
        x = torch.cat([x.view(N,-1), c],dim=1)
        tmp = self.encoder(x)
        mu = self.mu_layer(tmp)
        logvar = self.logvar_layer(tmp)
        z = reparametrize(mu,logvar)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    reconstruction_loss = F.binary_cross_entropy(x_hat, x, reduction='sum') / x.shape[0]
