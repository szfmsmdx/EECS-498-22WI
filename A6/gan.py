...
def sample_noise(batch_size, noise_dim, dtype=torch.float, device="cpu"):
    ...
    noise = 2*torch.rand(size=(batch_size,noise_dim),dtype=dtype, device=device)-1
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
...
def discriminator_loss(logits_real, logits_fake):
    ...
    real_loss = torch.nn.functional.binary_cross_entropy_with_logits(logits_real,labels_real)
    fake_loss = torch.nn.functional.binary_cross_entropy_with_logits(logits_fake,labels_fake)
...
def generator_loss(logits_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    lables_fake = torch.ones_like(logits_fake)
    loss = nn.functional.binary_cross_entropy_with_logits(logits_fake,lables_fake)
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    # Replace "pass" statement with your code
    loss = 0.5 * (
        torch.pow(scores_real-1,2).mean()+torch.pow(scores_fake,2).mean()
    )
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
