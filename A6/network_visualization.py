...
def compute_saliency_maps(X, y, model):
    ...
    loss.backward()
    saliency, _ = torch.abs(X.grad.data).max(dim=1)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return saliency
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for epoch in range(max_iter):
        X_adv.detach().requires_grad_()

        scores = model(X_adv)
        _, pre_class = torch.max(scores, dim=1)
        ...
        if verbose:
            print(f"Iteration# {epoch}, target score: {target_scores.item():.3f}, target class: {target_y}, predicted_class: {pre_class.item()}")
        if pre_class.item() == target_y:
            break
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    scores = model(img)
    target_scores = scores[0,target_y]
    loss = target_scores - l2_reg * torch.sum(img**2)
    loss.backward()
