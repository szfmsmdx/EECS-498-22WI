...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params['lateral_c3'] = nn.Conv2d(dummy_out['c3'].shape[1], self.out_channels, kernel_size=1, stride=1)
        self.fpn_params['lateral_c4'] = nn.Conv2d(dummy_out['c4'].shape[1], self.out_channels, kernel_size=1, stride=1)
        self.fpn_params['lateral_c5'] = nn.Conv2d(dummy_out['c5'].shape[1], self.out_channels, kernel_size=1, stride=1)
    ...
    def forward(self, images: torch.Tensor):
        ...
        fpn_feats['p4'] = self.fpn_params['lateral_c4'](backbone_feats['c4']) + F.interpolate(fpn_feats['p5'], scale_factor=2, mode='nearest')
        fpn_feats['p3'] = self.fpn_params['lateral_c3'](backbone_feats['c3']) + F.interpolate(fpn_feats['p4'], scale_factor=2, mode='nearest')
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    while idx.numel() > 0:
        ...
        if idx.numel() == 1:
            break
        iou = torch.tensor(
            [IoU(boxes[idx[0]], boxes[idx[i]]) for i in range(1, idx.numel())]
        )
