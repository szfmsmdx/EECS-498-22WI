...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(0, len(stem_cls), 2):
            nn.init.normal_(stem_cls[i].weight, mean=0, std=0.01)
            nn.init.zeros_(stem_cls[i].bias)
            nn.init.normal_(stem_box[i].weight, mean=0, std=0.01)
            nn.init.zeros_(stem_box[i].bias)
        ...
        self.pred_cls = nn.Conv2d(
            in_channels=channels[-1], out_channels=num_classes, kernel_size=3, stride=1, padding=1
        )
        self.pred_box = nn.Conv2d(
            in_channels=channels[-1], out_channels=4, kernel_size=3, stride=1, padding=1
        )
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feat in feats_per_fpn_level.items():
            pre_cls_out = self.pred_cls(self.stem_cls(feat))
            N, _, H, W = pre_cls_out.shape
            class_logits[level_name] = pre_cls_out.permute(0, 2, 3, 1).reshape(N, H * W, -1)
            
            shared_stem_box = self.stem_box(feat)
            pre_box_out = self.pred_box(shared_stem_box)
            boxreg_deltas[level_name] = pre_box_out.permute(0, 2, 3, 1).reshape(N, H * W, -1)

            pre_ctr_out = self.pred_ctr(shared_stem_box)
            centerness_logits[level_name] = pre_ctr_out.permute(0, 2, 3, 1).reshape(N, H * W, -1)
...
def fcos_get_deltas_from_locations(
    ...
    right[gt_boxes[:, 2] == -1] = -1
    bottom = (gt_boxes[:, 3] - locations[:, 1]) / stride
    bottom[gt_boxes[:, 3] == -1] = -1
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    left = deltas[:, 0].clip(0) * stride
    top = deltas[:, 1].clip(0)  * stride
    right = deltas[:, 2].clip(0) * stride
    bottom = deltas[:, 3].clip(0) * stride

    x1 = locations[:, 0] - left
    y1 = locations[:, 1] - top
    x2 = locations[:, 0] + right
    y2 = locations[:, 1] + bottom
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = torch.sqrt(
        torch.min(left, right) * torch.min(top, bottom) / (torch.max(left, right) * torch.max(top, bottom))
    )
    centerness[torch.any(deltas == -1, dim=-1)] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        fpn_feats_shapes = {
            leval_name: feat.shape for leval_name, feat in fpn_feats.items()
        }
        strides_per_fpn_level = self.backbone.fpn_strides
        locations_per_fpn_level = get_fpn_location_coords(
            fpn_feats_shapes, strides_per_fpn_level, device=images.device
        )
        ...
        for i in range(B):
            matched_gt_boxes.append(
                fcos_match_locations_to_gt(locations_per_fpn_level, strides_per_fpn_level, gt_boxes[i])
            )
        ...
        for i in range(B):
            ...
            for pi in matched_gt_boxes[i].keys():
                delta_dict[pi] = fcos_get_deltas_from_locations(
                    locations_per_fpn_level[pi], matched_gt_boxes[i][pi], strides_per_fpn_level[pi]
                )
                ctr_dict[pi] = fcos_make_centerness_targets(delta_dict[pi])
            matched_gt_deltas.append(delta_dict)
            matched_gt_crtness.append(ctr_dict)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        matched_gt_crtness = default_collate(matched_gt_crtness)

        # Combine predictions and GT from across all FPN levels.
        # shape: (batch_size, num_locations_across_fpn_levels, ...)
        matched_gt_boxes = self._cat_across_fpn_levels(matched_gt_boxes)
        matched_gt_deltas = self._cat_across_fpn_levels(matched_gt_deltas)
        matched_gt_crtness = self._cat_across_fpn_levels(matched_gt_crtness)
        ...
        matched_gt_crtness = matched_gt_crtness.view(-1)

        matched_gt_boxes_one_hot = nn.functional.one_hot(
            (matched_gt_boxes[:, :, -1] + 1).long(), self.num_classes + 1
        )

        loss_cls = sigmoid_focal_loss(pred_cls_logits, matched_gt_boxes_one_hot[:, :, 1:].float())
        loss_box = F.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction="none") / 4
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits, matched_gt_crtness, reduction="none")

        loss_box[matched_gt_deltas<0]=0
        loss_ctr[matched_gt_crtness<0]=0
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_boxes = fcos_apply_deltas_to_locations(
                level_deltas[idx], level_locations[idx], self.backbone.fpn_strides[level_name]
            )

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            H, W = images.shape[-2:]
            level_pred_boxes[:, 0] = level_pred_boxes[:, 0].clip(0, W)
            level_pred_boxes[:, 1] = level_pred_boxes[:, 1].clip(0, H)
            level_pred_boxes[:, 2] = level_pred_boxes[:, 2].clip(0, W)
            level_pred_boxes[:, 3] = level_pred_boxes[:, 3].clip(0, H)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
