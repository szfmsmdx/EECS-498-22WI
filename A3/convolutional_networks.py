...
class Conv(object):
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
            for f in range(F):
                for height in range(H_out):
                    for weight in range(W_out):
                      dw[f] += dout[n, f, height, weight] * x[n,:,height*stride:height*stride+HH,weight*stride:weight*stride+WW]
                      db[f] += dout[n, f, height, weight]
                      dx[n,:,height*stride:height*stride+HH,weight*stride:weight*stride+WW] += dout[n, f, height, weight] * w[f]

        dx = dx[:, :, pad:-pad, pad:-pad] # remove padding，forward 中的 padding 进行还原
        ###############################################################
        #                       END OF YOUR CODE                      #
        ###############################################################
        return dx, dw, db
    ...
    def forward(x, pool_param):
        ...
        for n in range(N):
            for c in range(C):
                for height in range(H_out):
                    for width in range(W_out):
                        out[n, c, height, width] = x[n, c, height*stride:height*stride+PH, width*stride:width*stride+PW].max()
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
            for c in range(C):
                for height in range(H_out):
                    for width in range(W_out):
                        window = x[n, c, height*stride:height*stride+PH, width*stride:width*stride+PW]
                        max_val = window.max()
                        dx[n, c, height*stride:height*stride+PH, width*stride:width*stride+PW] += (window == max_val) * dout[n, c, height, width]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Replace "pass" statement with your code
        conv_parm = {'stride': 1, 'pad': (filter_size - 1) // 2}
        pool_parm = {'pool_height': 2, 'pool_width': 2, 'stride': 2}
        C, H, W = input_dims
        HH = WW = filter_size
        H_out_conv = int(1 + (H + 2 * conv_parm['pad'] - HH) // conv_parm['stride'])
        W_out_conv = int(1 + (W + 2 * conv_parm['pad'] - WW) // conv_parm['stride'])
        H_out = int(1 + (H_out_conv - pool_parm['pool_height']) // pool_parm['stride'])
        W_out = int(1 + (W_out_conv - pool_parm['pool_width']) // pool_parm['stride'])
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        out, cov_relu_pool_cache = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        ...
        dout, dw, db = Linear.backward(dout, linear_cache)
        grads['W3'] = dw + 2 * self.reg * W3
        grads['b3'] = db
        dout, dw, db = Linear_ReLU.backward(dout, linear_relu_cache)
        grads['W2'] = dw + 2 * self.reg * W2
        grads['b2'] = db
        dout, dw, db = Conv_ReLU_Pool.backward(dout, cov_relu_pool_cache)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        # Replace "pass" statement with your code
        HH = WW = 3
        conv_parm = {'stride': 1, 'pad': 1}
        pool_parm = {'pool_height': 2, 'pool_width': 2, 'stride': 2}
        pre_filter, H_out, W_out = input_dims
        for i in range(self.num_layers-1):
            H_out = int(1 + (H_out + 2 * conv_parm['pad'] - HH) // conv_parm['stride'])
            W_out = int(1 + (W_out + 2 * conv_parm['pad'] - WW) // conv_parm['stride'])
            if self.batchnorm:
            ...
            if i in self.max_pools:
                H_out = int(1 + (H_out - pool_parm['pool_height']) // pool_parm['stride'])
                W_out = int(1 + (W_out - pool_parm['pool_width']) // pool_parm['stride'])
            if weight_scale == 'kaiming':
                self.params[f'W{i+1}'] = kaiming_initializer(pre_filter, num_filters[i], HH, relu=True, device=device, dtype=dtype)
            else:
                self.params[f'W{i+1}'] = weight_scale * torch.randn(num_filters[i], pre_filter, HH, WW, dtype=dtype, device=device)
            self.params[f'b{i+1}'] = torch.zeros(num_filters[i], dtype=dtype, device=device)
            pre_filter = num_filters[i]

        if weight_scale == 'kaiming':
            self.params[f'W{self.num_layers}'] = kaiming_initializer(pre_filter*H_out*W_out, num_classes, relu=False, device=device, dtype=dtype)
        else:
            self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(pre_filter*H_out*W_out, num_classes, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            if i in self.max_pools:
                if self.batchnorm:
                    out, cache_log[f'{i+1}'] = Conv_BatchNorm_ReLU_Pool.forward(
                    ...
                    out, cache_log[f'{i+1}'] = Conv_ReLU_Pool.forward(
                        out, self.params[f'W{i+1}'], self.params[f'b{i+1}'],
                        conv_param, pool_param
                    )
            else:
                if self.batchnorm:
                    out, cache_log[f'{i+1}'] = Conv_BatchNorm_ReLU.forward(
                        out, self.params[f'W{i+1}'], self.params[f'b{i+1}'],
                        self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'],
                        conv_param, self.bn_params[i]
                    )
                else:
                    out, cache_log[f'{i+1}'] = Conv_ReLU.forward(
                        out, self.params[f'W{i+1}'], self.params[f'b{i+1}'],
                        conv_param
                    )
        out, cache_log[f'{self.num_layers}'] = Linear.forward(
        ...
        # Replace "pass" statement with your code
        loss, dout = softmax_loss(scores, y)
        loss += self.reg * sum([torch.sum(self.params[f'W{self.num_layers}']**2)])
        dout, dw, db = Linear.backward(dout, cache_log[f'{self.num_layers}'])
        grads[f'W{self.num_layers}'] = dw + 2 * self.reg * self.params[f'W{self.num_layers}']
        grads[f'b{self.num_layers}'] = db
        for i in range(self.num_layers-2, -1, -1):
            if i in self.max_pools:
                if self.batchnorm:
                    loss += self.reg * (torch.sum(self.params[f'W{i+1}']**2) + torch.sum(self.params[f'gamma{i+1}']**2))
                    dout, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dout, cache_log[f'{i+1}'])
                    grads[f'gamma{i+1}'] = dgamma + 2 * self.reg * self.params[f'gamma{i+1}']
                    grads[f'beta{i+1}'] = dbeta
                    grads[f'W{i+1}'] = dw + 2 * self.reg * self.params[f'W{i+1}']
                    grads[f'b{i+1}'] = db
                else:
                    loss += self.reg * torch.sum(self.params[f'W{i+1}']**2)
                    dout, dw, db = Conv_ReLU_Pool.backward(dout, cache_log[f'{i+1}'])
                    grads[f'W{i+1}'] = dw + 2 * self.reg * self.params[f'W{i+1}']
                    grads[f'b{i+1}'] = db
            else:
                if self.batchnorm:
                    loss += self.reg * (torch.sum(self.params[f'W{i+1}']**2) + torch.sum(self.params[f'gamma{i+1}']**2))
                    dout, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dout, cache_log[f'{i+1}'])
                    grads[f'gamma{i+1}'] = dgamma + 2 * self.reg * self.params[f'gamma{i+1}']
                    grads[f'beta{i+1}'] = dbeta
                    grads[f'W{i+1}'] = dw + 2 * self.reg * self.params[f'W{i+1}']
                    grads[f'b{i+1}'] = db
                    ...
                    loss += self.reg * torch.sum(self.params[f'W{i+1}']**2)
                    dout, dw, db = Conv_ReLU.backward(dout, cache_log[f'{i+1}'])
                    grads[f'W{i+1}'] = dw + 2 * self.reg * self.params[f'W{i+1}']
                    grads[f'b{i+1}'] = db
...
def find_overfit_parameters():
    ...
    learning_rate = 1e-6  # Experiment with this!
    ###########################################################
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.randn(Din, Dout, dtype=dtype, device=device) * torch.sqrt(torch.tensor(gain / Din))
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.randn(Dout, Din, K, K, dtype=dtype, device=device) * torch.sqrt(torch.tensor(gain / (Din * K * K)))
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            var = 1. / N * torch.sum((x - mu) ** 2, dim=0)
            running_mean = momentum * running_mean + (1 - momentum) * mu
            running_var = momentum * running_var + (1 - momentum) * var
            x_hat = (x - mu) / torch.sqrt(var + eps)
            out = gamma * x_hat + beta
            cache = (x, mu, var, x_hat, gamma, beta, eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            ################################################################
            # Replace "pass" statement with your code
            normolized = ((x - running_mean) / torch.sqrt(running_var + eps))
    ...
    def backward(dout, cache):
        ...
        dx_hat = dout * gamma
        dvar = torch.sum(dx_hat * (x - mu) * -0.5 * (var + eps) ** -1.5, dim=0)
        # dmu = torch.sum(dx_hat * -1 / torch.sqrt(var + eps), dim=0) + dvar * torch.sum(-2 * (x - mu), dim=0) / N
        dmu = torch.sum(dx_hat * -1 / torch.sqrt(var + eps), dim=0)
        dx = dx_hat / torch.sqrt(var + eps) + dvar * 2 * (x - mu) / N + dmu / N
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        dgamma = torch.sum(dout * x_hat, dim=0)
        dbeta = torch.sum(dout, dim=0)
        ivar = 1. / torch.sqrt(var + eps)
        dx = (gamma*ivar/N) * (N*dout-x_hat*dgamma-dbeta)   # 全部转化为 dgamma 和 dbeta 的形式
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        pre_x = x.permute(1, 0, 2, 3).reshape(C, -1).T
        out, cache = BatchNorm.forward(pre_x, gamma, beta, bn_param)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        pre_x = dout.permute(1, 0, 2, 3).reshape(C, -1).T
        dx, dgamma, dbeta = BatchNorm.backward(pre_x, cache)
