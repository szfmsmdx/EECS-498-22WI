...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += self.reg * torch.sum(torch.sum(self.params['W1'] * self.params['W1']) + torch.sum(self.params['W2'] * self.params['W2']))
        dx, dw, db = Linear.backward(dout, cache_l)
        grads['W2'] = 2 * self.reg * self.params['W2'] + dw
        grads['b2'] = db
        
        dx, dw, db = Linear_ReLU.backward(dx, cache_lr)
        grads['W1'] = 2 * self.reg * self.params['W1'] + dw
        grads['b1'] = db
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers):
            self.params['W{}'.format(i)] = torch.normal(0, weight_scale, size=(last_dim, hidden_dims[i-1]), dtype=dtype, device=device)
            self.params['b{}'.format(i)] = torch.zeros(hidden_dims[i-1], dtype=dtype, device=device)
            last_dim = hidden_dims[i-1]
        self.params['W{}'.format(self.num_layers)] = torch.normal(0, weight_scale, size=(last_dim, num_classes), dtype=dtype, device=device)
        self.params['b{}'.format(self.num_layers)] = torch.zeros(num_classes, dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        cache_log = {}
        for i in range(1, self.num_layers):
            output, cache_log[f'cache{i}'] = Linear_ReLU.forward(output, self.params[f'W{i}'], self.params[f'b{i}'])
            if self.use_dropout:
                output, cache_log[f'cache_dropout{i}'] = Dropout.forward(output, self.dropout_param)
        output, cache_log[f'cache{self.num_layers}'] = Linear.forward(output, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
        if self.use_dropout:
            output, cache_log[f'cache_dropout{self.num_layers}'] = Dropout.forward(output, self.dropout_param)
        scores = output
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        loss, dout = softmax_loss(scores, y)
        if self.use_dropout:
            dout = Dropout.backward(dout, cache_log[f'cache_dropout{self.num_layers}'])
        dout, dw, db = Linear.backward(dout, cache_log[f'cache{self.num_layers}'])
        grads[f'W{self.num_layers}'] = 2 * self.reg * self.params[f'W{self.num_layers}'] + dw
        grads[f'b{self.num_layers}'] = db
        loss += self.reg * self.params[f'W{self.num_layers}'].pow(2).sum()
        for i in range(self.num_layers-1, 0, -1):
            if self.use_dropout:
                dout = Dropout.backward(dout, cache_log[f'cache_dropout{i}'])
            dout, dw, db = Linear_ReLU.backward(dout, cache_log[f'cache{i}'])
            grads[f'W{i}'] = 2 * self.reg * self.params[f'W{i}'] + dw
            grads[f'b{i}'] = db
            loss += self.reg * self.params[f'W{i}'].pow(2).sum()
...
def rmsprop(w, dw, config=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    square_dw = config['decay_rate'] * config['cache'] + (1 - config['decay_rate']) * dw * dw
    next_w = w - config['learning_rate'] * dw / (torch.sqrt(square_dw + config['epsilon']))
    config['cache'] = square_dw
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
