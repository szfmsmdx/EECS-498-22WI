...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    #         dists[i, j] = torch.sum((x_train[i] - x_test[j]) ** 2).sqrt()
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    # for i in range(num_train):
    #     dists[i] = torch.sum((x_train[i] - x_test) ** 2, dim=tuple(range(1, len(x_train.shape)))).sqrt()
    for i in torch.arange(num_train):
        x0 = x_train[i].flatten()
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # x_test.unsqueeze_(0)
    # dists = torch.sum((x_train - x_test) ** 2, dim=tuple(range(2, len(x_train.shape)))).sqrt()
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    top_min_k_index = torch.topk(dists, k, dim=0, largest=False).indices    # dim=0相当于对每一列进行topk
    y_pred = torch.mode(y_train[top_min_k_index], dim=0).values # mode返回众数，values返回众数的值
...
def knn_cross_validate(
    ...
    assert x_train.shape[0] % num_folds == 0    # 保证能够平均分成num_folds份
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            accuracy = classifier.check_accuracy(x_test, y_test, k)
