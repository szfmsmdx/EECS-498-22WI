...
def svm_loss_vectorized(
    ...
    # reshape(-1, 1) 没想到。。。
    correct_class_score = socore[row, y].reshape(-1,1)           # (N, 1)

    # margins = max(socore - correct_class_score + 1, 0)         # (N, C)
    # max(>0, 0) 没法过。。。只能这么写了
    margins = socore - correct_class_score + 1
    margins[margins < 0] = 0
    margins[row, y] = 0                                          
    loss = margins.sum() / num_train + reg * torch.sum(W * W)    # (D, C)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    # 我们知道 x^T 是(D, N)的，所以想得到和W相同形状的(D,C),需要乘上一个(N,C)的
    # 而这个矩阵表明了，每个样本的每个类别贡献的 x^T[i] 的次数
    cnt_matrix = torch.zeros(num_train, num_class, device=X.device, dtype=X.dtype)
    cnt_matrix[margins > 0] = 1
    cnt_matrix[row, y] = -torch.sum(cnt_matrix, dim=1)
    
    dW = X.t().mm(cnt_matrix)
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    sample_indics = torch.randint(0, num_train, (batch_size, ))
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [6e-3, 8e-3, 1e-2, 2e-2, 3e-2]
    regularization_strengths = [5e-4, 8e-4, 1e-3, 2e-3]        
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i]) # (C, )
        # 是向量所以直接用。。。
        scores -= torch.max(scores) # 不影响理论结果，但是对精度有影响
        loss -= torch.log(torch.exp(scores[y[i]]) / torch.sum(torch.exp(scores)))
        for j in range(num_class):
            dW[:, j] += X[i] * (torch.exp(scores[j])) / torch.sum(torch.exp(scores))
        dW[:, y[i]] -= X[i]
...
def softmax_loss_vectorized(
    ...
    # 同样设置一个中间概率矩阵 (N, D)
    # view(-1, 1) 是为了让形状变成(N, 1) 方便广播
    probs = torch.exp(scores) / torch.sum(torch.exp(scores), dim=1).view(-1, 1)  # (N, C)

    loss = -torch.sum(torch.log(probs[torch.arange(num_train), y]))
    loss /= num_train
    loss += reg * torch.sum(W * W)

    probs[torch.arange(num_train), y] += -1 # dW[:, y[i]] -= X[i]
    dW = X.t().mm(probs)
    dW /= num_train
    dW += 2 * reg * W
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [2e1, 8e1, 2e2]
    regularization_strengths = [1e-4, 4e-4, 8e-4]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
