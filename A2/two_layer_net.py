...
def nn_forward_backward(
    ...
    grads['W2'] = h1.t().mm(probs)
    grads['W2'] += 2 * reg * W2
    # 实际上 b2 应该是个和 scores 同形状的矩阵，然而，这里是单纯的一个偏置
    # 说明是利用了广播机制，将每个维度都加到一起了。。。所以是叠加效果
    grads['b2'] = torch.sum(probs, dim=0)   
    
    grads_h1 = probs.mm(W2.t())   # 传递导数到 h1
    grads_h1[h1 <= 0] = 0        # 对 relu 求导
    
    grads['W1'] = X.t().mm(grads_h1)
    grads['W1'] += 2 * reg * W1
